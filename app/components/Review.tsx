...
const Review = ({productId,userId}: Props) => {
    ...
    return (
        <div>
            <h1 className='text-xl font-medium mb-4'>Give your Reviews</h1>
            ...
            <h2  className='mt-4'>Write your Reviews</h2>
            <div>
                <input type='text' className='border-[1px] border-purple-300 rounded-lg w-full h-[40px] focus:border-purple-500 outline-none px-2 p-2 mt-2'
                name='comment'
                onChange={handleChange}
                value={reviewForm.comment}
                />
            </div>
            <button className='px-5 p-2 corder-[1px] bg-purple-600 text-white rounded-lg mt-5'
            onClick={postData}
            >Submit</button>
        </div>  
    )
